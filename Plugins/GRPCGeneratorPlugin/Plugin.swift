...
struct GRPCGeneratorPlugin {
  ...
  func createBuildCommands(
    ...
    for inputFile in inputFiles {
      guard let configFile = findApplicableConfigFor(file: inputFile, from: configs.keys.map{ $0 } ) else {
      ...
      let protoDirectoryPath = configFile.deletingLastPathComponent()
...
func readConfigurationFiles(_ configurationFiles: [URL], pluginWorkDirectory: URL) throws -> [URL : CommonConfiguration] {
  ...
  for configFile in configurationFiles {
    ...
    config.fileNaming = .fullPath // hard-code full-path to avoid collisions since this goes into a temporary directory anyway
    config.outputPath = String(pluginWorkDirectory.relativePath) // the output directory mandated by the plugin system
...
func findApplicableConfigFor(file: URL, from configFiles: [URL]) -> URL? {
  ...
  for endComponent in (0..<filePathComponents.count).reversed() {
    for configFile in configFiles {
      if filePathComponents[..<endComponent] == configFile.pathComponents[..<(configFile.pathComponents.count-1)] {
        return configFile
      }
    }
  }
...
func protocGenGRPCSwiftCommand(inputFile: URL, configFile: URL, config: CommonConfiguration, protoDirectoryPath: URL, protocPath: URL, protocGenGRPCSwiftPath: URL) throws -> PackagePlugin.Command {
...
func protocGenSwiftCommand(inputFile: URL, configFile: URL, config: CommonConfiguration, protoDirectoryPath: URL, protocPath: URL, protocGenSwiftPath: URL) throws -> PackagePlugin.Command {
  guard let fileNaming = config.fileNaming else {
    assertionFailure("Missing file naming strategy - should be hard-coded.")
    throw PluginError.missingFileNamingStrategy
  }
...
extension GRPCGeneratorPlugin: XcodeBuildToolPlugin {
  ...
  func createBuildCommands(context: XcodePluginContext, target: XcodeTarget) throws -> [Command] {
    let configFiles = target.inputFiles.filter{ $0.url.lastPathComponent == "grpc-swift-config.json" }.map{ $0.url }
    let inputFiles = target.inputFiles.filter{ $0.url.lastPathComponent.hasSuffix(".proto") }.map{ $0.url }
    return try createBuildCommands(
      pluginWorkDirectory: context.pluginWorkDirectoryURL,
      tool: context.tool,
      inputFiles: inputFiles,
